...
// 1. Botón para mostrar TODAS
document.querySelector('#btn-todos').addEventListener('click', () => {
  dibujar(propiedades); // Dibuja el array completo
});

// 2. Botón para casas de menos de 200.000€
document.querySelector('#btn-baratas').addEventListener('click', () => {
  const baratas = propiedades.filter((p) => p.precio < 200000);
  dibujar(baratas); // Dibuja solo las filtradas
});

// 3. Botón para casas de lujo (más de 400.000€)
document.querySelector('#btn-lujo').addEventListener('click', () => {
  const caras = propiedades.filter((p) => p.precio > 400000);
  dibujar(caras); // Dibuja solo las filtradas
});

// IMPORTANTE: Llama a dibujar al principio para que la web no empiece vacía
dibujar(propiedades);
